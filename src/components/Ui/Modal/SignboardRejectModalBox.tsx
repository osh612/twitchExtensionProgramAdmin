...
import SignBoardServices, { ISignboardReject } from '../../../services/SignBoardServices';
...
const SignboardRejectModalBox = ({
  ...
  const onConfirm = () => {
    const param: ISignboardReject = {
      idx,
      userId,
      rejectCause: reason,
    };
    SignBoardServices.rejectSignBoard(param);
    CancelHandler();
  };

  return (
    <Box sx={MODAL_STYLE}>
      <Header>{t('signboard.label.reject')}</Header>
      <Body>
        {/* ID */}
        <InputBox>
...
export default SignboardRejectModalBox;
