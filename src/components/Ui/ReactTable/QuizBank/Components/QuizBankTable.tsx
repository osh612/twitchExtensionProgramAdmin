...
import { IQuizBankData } from '../makeQuizBankData';
import { ITableHeader } from '../../ReactTableLib';
...
const getFunctionIcon = (id: string, value: boolean, idx: number, uid: string) => {
  if (id === 'edit') {
    return <EditQuizButton idx={idx} selectedUid={uid} />;
  }
  if (id === 'delete') {
    return <DeleteQuizButton selectedUid={uid} idx={idx} />;
  }
  return <></>;
};
...
const QuizBankTable = ({
  ...
  columns: Column<IQuizBankData>[];
  data: IQuizBankData[];
  setData: React.Dispatch<React.SetStateAction<IQuizBankData[]>>;
  ...
  const neeeIcon = (id: string) => {
    if (['edit', 'delete'].includes(id)) return true;
    return false;
  };
