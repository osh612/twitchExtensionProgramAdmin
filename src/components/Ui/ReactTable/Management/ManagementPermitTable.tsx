...
import makeAccountPermitData from './makeAccountPermitData';
...
function Table({
  ...
  const getFunctionIcon = (id: string, value: boolean, idx: number, values: any) => {
    ...
    // if (id === 'edit') {
    //   return <EditAccountButton uid={values.id} />;
    // }
    // if (id === 'delete') {
    //   return <DeleteAccountButton idx={idx} values={values} />;
    // }
    if (id === 'checkAccept') {
      return <JoinAcceptButton values={values} getDataList={getDataList} />;
    }
    return <></>;
  };
...
const ManagementTable = ({
  ...
  // 탭에 맞는 테이블 셋팅
  const { no, id, email, broadcastType, grade, joinDate, checkAccept, leagueList, state, idx } =
    columnsDataset;
  showItems = [no, id, email, broadcastType, grade, joinDate, checkAccept];
  hideItems = [leagueList, state, idx];

  arr = arr.concat(showItems); // show
  arr = arr.concat(hideItems); // hide
  const columns = React.useMemo(() => arr, [accountList]);
  const data = React.useMemo(() => makeAccountPermitData(accountList), [accountList]);
